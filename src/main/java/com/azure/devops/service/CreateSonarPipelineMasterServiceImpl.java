...
@Slf4j
@Service
public class CreateSonarPipelineMasterServiceImpl {

    public ResponseEntity<?> createSonarMasterPipeline(RequestCreateProjectDTO requestCreateProjectDTO) {

        log.info("Starting the creation of sonar-master pipeline.");
        ...
        try {
            ...
            configurationPipelineDTO.setPath("azure-pipelines-sonar-master.yml");
            ...
            createPipelineDTO.setName("Sonar Master");

            response = restTemplate.postForEntity(uri, createPipelineDTO, String.class);
            log.info("Finalize the creation of pipeline sonar-master.");
