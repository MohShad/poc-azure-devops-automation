...
@Slf4j
@Service
public class CreatePRPipelineServiceImpl {

    public ResponseEntity<?> createPRPipeline(RequestCreateProjectDTO requestCreateProjectDTO) {

        log.info("Starting the creation of pull-request pipeline.");
        ...
        try {
            ...
            configurationPipelineDTO.setPath("azure-pipelines-pr.yml");
            ...
            createPipelineDTO.setName("Pull Request");

            response = restTemplate.postForEntity(uri, createPipelineDTO, String.class);
            log.info("Finalize the creation of pull request pipeline.");
